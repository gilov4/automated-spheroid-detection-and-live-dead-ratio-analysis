...
macro "Spheroid Detection and Live/Dead Ratio Analysis - Merged tif Analysis" {
    ...
    dir = getDirectory("Choose a directory containing the merged image files");
    ...
    for (i = 0; i < list.length; i++) {
        if (startsWith(list[i], "Merged_") && endsWith(list[i], filter)) {
            processImage(dir, list[i], useGPU); // Process each valid image
        }
    }
...
function processImage(dir, fileName, useGPU) {
    print("--------------------------------------------------------------------------------");
    ...
    roiManager("Save", dir + File.separator + orgName + ".zip"); // Save ROIs for future reference
    ...
    roiManager("Deselect");
    roiManager("Delete");
    resultsPath = dir + File.separator + csvName + "_Live_Dead_Results.csv";
